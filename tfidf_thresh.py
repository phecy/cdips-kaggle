...
def calc_tfidf(feat_mat,featureIndex):
    ...
    print 'Ngrams, no uniform zero cols (nor 1-element cols)',ngram_mat.shape,len(nzIndex)
    # Calculate TF-IDF --NOTE: Prints off-by-one column drop, but matrix is correct shape
    tfidf = DimReduction(ngram_mat,'tfidf')
    print 'TFIDF',tfidf.shape,'previous:',ngram_mat.shape,len(nzIndex)
