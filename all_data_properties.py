...
filename = "C:\\Users\\Cory\\Documents\\DataScienceWorkshop\\avito_kaggle\\avito_train.tsv"

samp_data = pd.read_csv(filename, sep='\t')

print "total number of analyzed ads = " + str(len(samp_data)) + "\n"

print "total number of illegitimate ads = " + str(len(samp_data[samp_data.is_blocked==1])) + "\n"

print "total number of legitimate ads = " + str(len(samp_data[samp_data.is_blocked==0])) + "\n"
...
#Create histogram for the price
n, bins, patches = ax.hist([samp_data.price[samp_data.is_blocked==0],samp_data.price[samp_data.is_blocked==1]],bins=range(0,10000,500),normed="TRUE",color=['blue','green'])
...
#price_proved = samp_data.price[(samp_data.is_blocked==1)]
#price_legit = samp_data.price[(samp_data.is_blocked==0)]
...
n, bins, patches = ax2.hist([samp_data.emails_cnt[samp_data.is_blocked==0],samp_data.emails_cnt[samp_data.is_blocked==1]],bins=[0,1,2,3],normed="TRUE",color=['blue','green'])
...
n, bins, patches = ax3.hist([samp_data.urls_cnt[samp_data.is_blocked==0],samp_data.urls_cnt[samp_data.is_blocked==1]],bins=[0,1,2,3],normed="TRUE",color=['blue','green'])
...
n, bins, patches = ax4.hist([samp_data.phones_cnt[samp_data.is_blocked==0],samp_data.phones_cnt[samp_data.is_blocked==1]],bins=[0,1,2,3],normed="TRUE",color=['blue','green'])
...
n, bins, patches = ax5.hist([samp_data.close_hours[samp_data.is_blocked==0],samp_data.close_hours[samp_data.is_blocked==1]],bins=range(0,20,1),normed="TRUE",color=['blue','green'])
ax5.legend(["Proven Legitimate","Proven Illicit"])
fig5.suptitle("Closing Time of Ad",fontsize=24)
plt.show()

t, prob = sp.stats.ttest_ind(samp_data.close_hours[samp_data.is_blocked==0],samp_data.close_hours[samp_data.is_blocked==1])
print "mean closing time of illegitimate = " + str(np.mean(samp_data.close_hours[samp_data.is_blocked==0]))
print "std closing time of illegitimate = " + str(np.std(samp_data.close_hours[samp_data.is_blocked==0])) + "\n"

print "mean closing time of legit = " + str(np.mean(samp_data.close_hours[samp_data.is_blocked==1]))
print "std closing time of legit = " + str(np.std(samp_data.close_hours[samp_data.is_blocked==1])) + "\n"
